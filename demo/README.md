...
PRODUCT_N_PERM_PB --seed 0` (`src/pbcheck/audit.py`'s own product constants, named here rather than
typed out so that their values live in one place; the committed `pbcheck_audit.json` records what was
actually used under `settings.tool` as `n_perm_requested`, `n_perm_pb_requested` and `seed`).
