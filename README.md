...
Two committed demonstrations under `demo/` run outside the pre-registered Phase 0 protocol. One
(Kang et al. 2018) completes against a real public dataset, and the tool's own design audit stops
that run because the dataset uses a paired design. The other was attempted against a second real
public dataset (Stephenson et al. 2021) that turned out to carry no raw integer counts anywhere in
its file; the committed run is a synthetic fallback (`example_reference`) instead, generated by
this repository's own code, not a third-party dataset. Both are labelled as demonstrations, not as
a Phase 0 measurement, and state no result about the underlying publications. See
...
demo/                 # v0.1.0 demonstrations, outside the protocol: one on real public data
                       # (Kang 2018), one a synthetic fallback (example_reference)
