...
- Added: demonstrations under `demo/` on public datasets outside the frozen Phase 0 list
  (Kang et al. 2018, stopped by the design gate as a paired design; a two-arm run attempted on
  Stephenson et al. 2021, stopped because no raw counts exist anywhere in that file, with a
  synthetic `example_reference` run committed instead), generated by `scripts/demo_*.py`;
  labelled demonstrations outside the pre-registered protocol.
