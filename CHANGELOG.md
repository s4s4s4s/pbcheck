...
[Keep a Changelog](https://keepachangelog.com/en/1.1.0/). Releases are tagged `vX.Y.Z`; the
pre-release history is kept under its own heading, grouped by the project's actual epochs of work
rather than by commit date, see `git log` for the literal timeline.
...
audit runs outside the pre-registered protocol. This release changes no number that
`scripts/synthetic_gate.py` prints, touches no frozen Phase 0 material, and claims no
Phase 0 result.

- Added: `pbcheck audit` CLI and `python -m pbcheck`: design audit with hard gates for paired
  designs and too few donors, raw-count check, frozen gene universe, naive per-cell and
  donor-pseudobulk arms corrected over one universe, donor-permutation null with lambda,
  labelled permutation floors and Monte-Carlo SEs, a plain-language read-out with fixed
  caveats and a glossary; outputs `pbcheck_audit.json` (schema `pbcheck-audit/1`),
  `pbcheck_report.md`, `pbcheck_report.html`.
- Added: `pbcheck example` (offline quickstart data) and `--design-only` metadata mode on any
  `.h5ad`; on non-integer counts the pseudobulk arm is dropped, never rounded, and the naive
  arm runs with a caveat and no negative control.
- Added: Python API `pbcheck.run_audit`, `pbcheck.AuditSettings`, `pbcheck.audit_h5ad`,
  `pbcheck.example_adata`, `pbcheck.render.render_markdown` / `render_html` / `write_outputs`.
- Added: `release.yml` (build, TestPyPI/PyPI trusted publishing, GitHub Release), `.zenodo.json`,
  `docs/USAGE.md`, `scripts/changelog_section.py`, `scripts/check_docstring_only_diff.py`,
  `scripts/compare_gate_scalars.py`, and the tests `test_audit.py`, `test_render.py`,
  `test_cli.py`, `test_example.py`, `test_packaging.py`, `test_docs.py`.
...
- Unchanged: `docs/PHASE0_SPEC.md`, `docs/AMENDMENTS.md`, `pilot/preregistration`, `pilot/gate`,
  `pilot/testsel`, `gate_config`, every engine module and every pre-existing test.

## Pre-release history (unreleased epochs, 2026-07-22 to 2026-09-05)
